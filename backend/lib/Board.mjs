...
import { getPieceValue, isPieceValid, isLocationValid } from "./utils.mjs";
...
export default class Board {
  ...
  getMoves(color = this.getPlayingColor(), movablePiecesRequiredToSkipTest = null) {
    ...
    if (!Object.keys(moves).length) {
      ...
      if (this.getNonPlayingColor() === COLORS.BLACK) {
        this.configuration.points = 0;
      }
      if (this.getNonPlayingColor() === COLORS.WHITE) {
        this.configuration.playerWon = true;
      }
    }
  ...
  exportJson() {
    return {
      ...
      playerWon: this.configuration.playerWon,
  ...
  calculatePoints(level) {
    let points = 0;
    const legalPieces = ["Q", "R", "R", "B", "B", "N", "N", "P", "P", "P", "P", "P", "P"];
    for (const location in this.configuration.pieces) {
      const piece = this.getPiece(location);
      switch (piece) {
        case "Q":
          if (legalPieces.includes("Q")) {
            points += 9;
            legalPieces.splice(legalPieces.indexOf("Q"), 1);
          }
          break;
        case "R":
          if (legalPieces.includes("R")) {
            points += 5;
            legalPieces.splice(legalPieces.indexOf("R"), 1);
          }
          break;
        case "B":
          if (legalPieces.includes("B")) {
            points += 3;
            legalPieces.splice(legalPieces.indexOf("B"), 1);
          }
          break;
        case "N":
          if (legalPieces.includes("N")) {
            points += 3;
            legalPieces.splice(legalPieces.indexOf("N"), 1);
          }
          break;
        case "P":
          if (legalPieces.includes("P")) {
            points += 1;
            legalPieces.splice(legalPieces.indexOf("P"), 1);
          }
      }
    }
      ...
      moves[from].map((to) => {
        ...
        scoreTable.push({
          ...
          score:
            ...
            Math.floor(
              Math.random() *
                (this.configuration.halfMove > 10 ? this.configuration.halfMove - 10 : 1) *
                10
            ) /
              10,
        });
  ...
  calculateScoreByPiecesLocation(player = this.getPlayingColor()) {
    ...
    for (const location in this.configuration.pieces) {
      ...
      if (scoreByPosition[piece]) {
        ...
        score +=
          (this.getPieceColor(piece) === player ? scoreIndex : -scoreIndex) * scoreMultiplier;
      }
    }
    return score;
  }
